...
class MlArrayStorage:
    ...
    def filter_array(self, curtime, deltatime):
        for i in range(len(self.id_index) - 1, -1, -1):
            if (curtime - self.id_time[i][-1]) > deltatime and self.id_array_y[i][
                -1
            ] != -1:
                del self.id_index[i]
                del self.id_bsm[i]
    ...
    def get_bsm_array(self, bsmNew, bsmOld):
        ...
        DeltaPos = self.get_distance(
            bsmNew["BsmPrint"]["BSMs"][0]["Pos"], bsmOld["BsmPrint"]["BSMs"][0]["Pos"]
        )
        ...
        DeltaSpeed = self.get_speed(
            bsmNew["BsmPrint"]["BSMs"][0]["Speed"]
        ) - self.get_speed(bsmOld["BsmPrint"]["BSMs"][0]["Speed"])
        SpeedConfidence = self.get_speed(
            bsmNew["BsmPrint"]["BSMs"][0]["SpeedConfidence"]
        )
        Accel = self.get_speed(bsmNew["BsmPrint"]["BSMs"][0]["Accel"])
        DeltaAccel = self.get_speed(
            bsmNew["BsmPrint"]["BSMs"][0]["Accel"]
        ) - self.get_speed(bsmOld["BsmPrint"]["BSMs"][0]["Accel"])
        AccelConfidence = self.get_speed(
            bsmNew["BsmPrint"]["BSMs"][0]["AccelConfidence"]
        )
        DeltaHeading = self.get_angle_delta(
            bsmNew["BsmPrint"]["BSMs"][0]["Heading"],
            bsmOld["BsmPrint"]["BSMs"][0]["Heading"],
        )
        HeadingConfidence = self.get_speed(
            bsmNew["BsmPrint"]["BSMs"][0]["HeadingConfidence"]
        )
        DeltaTime = (
            bsmNew["BsmPrint"]["Metadata"]["generationTime"]
            - bsmOld["BsmPrint"]["Metadata"]["generationTime"]
        )
        ...
        velAry = np.concatenate(
            (np.array(velAry1.tolist()), np.array(velAry2), np.array(velAry3)),
            axis=None,
        )
    ...
    def get_array(self, id):
        index = self.id_index.index(id)
        return [self.id_array_x[index][-1][:30], self.id_array_y[index][-1]]
